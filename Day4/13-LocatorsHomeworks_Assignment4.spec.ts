...
test(`Test to launch a browser`, async () => {
   ...
   // await expect(lastName).toContainText("Sridharan");
    ...
    const dropdownValue = await page.getByRole(`combobox`);
    await dropdownValue.selectOption("Mr");
    await page.getByPlaceholder(`First Name`).fill("Saranya");
    await page.locator(`//span[contains(text(),'Save')]/parent::button[@class='slds-button slds-button_neutral uiButton--brand uiButton forceActionButton']`).click();
    await page.waitForTimeout(2000);
